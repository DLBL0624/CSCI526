...
public class HexGridChunk : MonoBehaviour
{
    ...
    void Triangulate(HexCell cell)
    {
        ...
        if (!cell.IsUnderwater)
        {
            ...
            if (cell.IsGreatWall)
            {
                features.AddSpecialWall(cell, cell.Position);
            }
        }
    }
    ...
    void TriangulateWater(HexDirection direction, HexCell cell, Vector3 center)
    {
            ...
            )
        {
            ...
            if (cell.HasRiverBeginOrEnd)
            {
                roadCenter += HexMetrics.GetSolidEdgeMiddle(cell.HasRiverBeginOrEndDirection.Opposite()) * (1f / 3f);
            }
        ...
        void TriangulateConnection(HexDirection direction, HexCell cell, EdgeVertices e1)   //三角化连接处
        {
            HexCell neighbor = cell.GetNeighbor(direction);
            if (neighbor == null)
            {
        ...
        void TriangulateCorner(Vector3 bottom, HexCell bottomCell, Vector3 left, HexCell leftCell, Vector3 right, HexCell rightCell)
        {
            ...
            else if (rightEdgeType == HexEdgeType.Slope)
            {
                ...
                {
                    TriangulateCornerTerracesCliffTerraces(
                        bottom, bottomCell, left, leftCell, right, rightCell
                    );
                }
            }
            else if (leftCell.GetEdgeType(rightCell) == HexEdgeType.Slope)
            {
                ...
                {
                    TriangulateCornerTerracesCliffTerraces(right, rightCell, bottom, bottomCell, left, leftCell);
                }
                else
                {
        ...
        void TriangulateCornerTerracesCliffTerraces(Vector3 begin, HexCell beginCell, Vector3 left, HexCell leftCell, Vector3 right, HexCell rightCell)
        {
